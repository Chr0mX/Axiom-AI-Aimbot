...
- **Auto Un-ADS** (`auto_unads_enabled`) — single-clicks a **dedicated** key/button (`auto_unads_key` generically, or `auto_unads_makcu_button` when `mouse_move_method == 'makcu'` — deliberately *not* `config.AimKeys`/`makcu_aim_button`, since it's most commonly a distinct in-game "toggle ADS" bind) when the selected target's box exceeds `auto_unads_box_threshold_pct` of `detect_range_size` ("too close") or its frame-to-frame speed exceeds `auto_unads_velocity_threshold_px_s` ("too fast" — `0` disables this half of the gate), then clicks it again once both conditions clear for `auto_unads_reengage_debounce_s`, or `auto_unads_max_release_s` elapses regardless (safety cap). The point is letting the wider hipfire view reacquire a target ADS zoom is about to lose. A *single click* (down+up), not a sustained hold, is fired on both the release and re-engage transitions — that's what makes it correct whether the user's in-game ADS bind is a toggle key (needs a fresh press edge each flip) or a hold key (a click's final state is released, same as tapping and letting go). Because this key is separate from `AimKeys`/`makcu_aim_button`, it never touches the state `is_aiming` is computed from — no override or held-VK snapshot is needed in `ai_loop.py` for this feature. Decision logic lives in `process_aiming()` (sets `state.unads_pending_transition`, gates the final `send_mouse_move` call only — sticky lock/PID/prediction keep running through the release window); the actual click is fired by `ai_loop_utils.apply_unads_transition()`, called **unconditionally** every `ai_loop.py` iteration (not only when `process_aiming()` ran) so a full target loss can still force a re-engage. Click dispatch: MAKCU goes through `makcu_mouse.press_button(button, action=1)` (device-side click command, all five buttons); the generic path goes through `win_utils/aim_input.py`'s `click_aim_key()` (mouse buttons via `win32api.mouse_event` down+up, plain keyboard VKs via `keybd_event` down+up — keyboard injection is independent of `mouse_move_method`; gamepad VKs and unsupported `mouse_move_method`s log-once and no-op).
...
**Aim page Anti-Recoil section** contains: Smart Jitter enable toggle, LMB gate, jitter strength, box threshold, "Record Jitter" push-button (inline toggle: ● Record / ■ Stop & Save), and a "Recorded Pattern" combo that lists all `jitter_patterns/*.json` files. A separate **Auto Un-ADS section** immediately follows it: enable toggle, box-size and velocity release thresholds, re-engage debounce, and max-release safety cap — see the Aiming & Anti-Recoil architecture note above. The dedicated key/button itself is configured separately, on the **Keys & HW page**'s "Auto Un-ADS Key" group (a `KeyBindButton` for the generic path, a MAKCU button combo for the MAKCU path — whichever is relevant is shown/hidden the same way the existing aim-key cards already switch on `mouse_move_method`).
...
| `auto_unads_enabled` | `aim.auto_unads.enabled` | Enable Auto Un-ADS (click a dedicated key on close/fast targets) |
| `auto_unads_key` | `aim.auto_unads.key` | Generic-path VK Auto Un-ADS clicks; `0` = unbound |
| `auto_unads_makcu_button` | `aim.auto_unads.makcu_button` | MAKCU-path button Auto Un-ADS clicks (`left`/`right`/`middle`/`side1`/`side2`/`off`) |
| `auto_unads_box_threshold_pct` | `aim.auto_unads.box_threshold_pct` | % of detect range above which target box height triggers a release click |
| `auto_unads_velocity_threshold_px_s` | `aim.auto_unads.velocity_threshold_px_s` | Target px/s above which triggers a release click; `0` disables this trigger |
| `auto_unads_reengage_debounce_s` | `aim.auto_unads.reengage_debounce_s` | Seconds the clear condition must hold before re-clicking to re-engage |
| `auto_unads_max_release_s` | `aim.auto_unads.max_release_s` | Safety cap forcing a re-engage click regardless of target state; `0` = no cap |
