...
- **Auto Un-ADS** (`auto_unads_enabled`) — synthetically releases the user's *configured* aim key/button (`config.AimKeys` / `makcu_aim_button` — no separate keybind) when the selected target's box exceeds `auto_unads_box_threshold_pct` of `detect_range_size` ("too close") or its frame-to-frame speed exceeds `auto_unads_velocity_threshold_px_s` ("too fast" — `0` disables this half of the gate), then re-presses it once both conditions clear for `auto_unads_reengage_debounce_s`, or `auto_unads_max_release_s` elapses regardless (safety cap). The point is letting the wider hipfire view reacquire a target ADS zoom is about to lose. Decision logic lives in `process_aiming()` (sets `state.unads_pending_transition`, gates the final `send_mouse_move` call only — sticky lock/PID/prediction keep running through the release window); the actual synthetic press/release is applied by `ai_loop_utils.apply_unads_transition()`, called **unconditionally** every `ai_loop.py` iteration (not only when `process_aiming()` ran) so a full target loss can still force a re-engage. `ai_loop.py` also overrides `is_aiming = True` while a release is active: for MAKCU this is a no-op (`rmb_held`/`lmb_held` read a raw-physical button stream unaffected by the app's own `right()`/`left()` writes — see `docs/MAKCU_Native_API.md`), but for the generic `GetAsyncKeyState`-based backends the synthetic release genuinely flips `is_key_pressed()` false, so without the override `process_aiming()` would stop being invoked mid-window. Synthetic send lives in `win_utils/aim_input.py`'s `set_aim_key_state()` (mouse buttons via `sendinput`/`mouse_event`/`makcu`; plain keyboard VKs via `keybd_event`; gamepad-VK aim keys and `ddxoft`/`arduino`/`xbox` mouse-move methods log-once and no-op — no release primitive implemented for those). **Known limitation**: on the generic backends, if the user genuinely releases the real key/button *during* a release window, the later synthetic re-press can leave `GetAsyncKeyState` reading "held" until one more real press+release cycle — MAKCU is immune (raw physical telemetry); the GUI recommends MAKCU for reliable behavior.
...
| `auto_unads_enabled` | `aim.auto_unads.enabled` | Enable Auto Un-ADS (release aim key on close/fast targets, reuses `AimKeys`/`makcu_aim_button`) |
| `auto_unads_box_threshold_pct` | `aim.auto_unads.box_threshold_pct` | % of detect range above which target box height triggers a release |
| `auto_unads_velocity_threshold_px_s` | `aim.auto_unads.velocity_threshold_px_s` | Target px/s above which triggers a release; `0` disables this trigger |
| `auto_unads_reengage_debounce_s` | `aim.auto_unads.reengage_debounce_s` | Seconds the clear condition must hold before re-pressing the aim key |
| `auto_unads_max_release_s` | `aim.auto_unads.max_release_s` | Safety cap forcing re-engage regardless of target state; `0` = no cap |
